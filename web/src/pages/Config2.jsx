...
const Config2 = ({ onClose }) => {
    const {
        ...
        moveDevice,
        devices,
        globalSettings,       // New: from Context
        updateGlobalSettings, // New: from Context
        isESP32               // New: detection from Context
    } = useESPContext();
    ...
    return (
        <div className="config-page font-base">
            ...
            <section className="config-section mb-6">
                ...
                <div className="section-title">
                    <label>Home Assistant Host (optional)</label>
                    <input
                        className="config-input"
                        placeholder="e.g. ha.mydomain.com or 192.168.1.100:8123"
                        value={globalSettings.haHost || ''}
                        onChange={(e) => updateGlobalSettings({ haHost: e.target.value })}
                    />
                </div>
            </section>
            ...
            <section className="config-section">
                ...
                {masterList.map(dev => (
                    <div key={dev.id} className="hardware-grid row">
                        {/* Use the Buffered Input for Host */}
                        <EditableConfigInput
                            value={dev.host}
                            onSave={(newVal) => updateDeviceConfig(dev.id, { host: newVal })}
                        />

                        {/* Use the Buffered Input for Name */}
                        <EditableConfigInput
                            value={dev.name}
                            onSave={(newVal) => updateDeviceConfig(dev.id, { name: newVal })}
                        />
                        <button onClick={() => deleteDevice(dev.host)} className="icon-btn text-red">
                            <Trash2 size={16} />
                        </button>
                    </div>
                ))}
            ...
            <section className="config-section mt-8">
                ...
                {masterList.map(dev => (
                    <div key={dev.id + "-fleet"} className={`fleet-grid row ${selectedUpdateHost === dev.host ? 'selected' : ''}`}>
                        {/* 1. Visibility */}
...
// Helper component extracted for cleanliness
const EditableConfigInput = ({ value, onSave, placeholder }) => {
    // 1. Keep a local "Draft" state so typing is smooth and lag-free
    const [draft, setDraft] = useState(value);

    // 2. If the global value changes (e.g. from a sync), update our draft
    useEffect(() => {
        setDraft(value);
    }, [value]);
    ...
    return (
        <input
            ...
            onChange={(e) => setDraft(e.target.value)} // ONLY updates local state
            onBlur={() => {
                // 3. ONLY save to the Master List when user clicks away
                if (draft !== value) {
                    onSave(draft);
                }
            }}
